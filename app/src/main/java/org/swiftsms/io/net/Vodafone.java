...
import org.swiftsms.utils.HTMLParser;
	...
	private OperationResult sendMessage(final List<String> recipients, final String message) {
		...
		while (sendHtml.contains(SEND_CAPTCHA_TEXT_INCORRECT)) {
			...
			if (this.isCaptchaRequired) {
				...
				if (captcha.isEmpty()) {
					return new OperationResult(Status.CANCELLED, R.string.message_cancelled);
				}
	...
	private void displayCaptchaDialog() {
		this.handler.post(new Runnable() {
			...
			public void run() {
				...
				final Builder builder = new Builder(Vodafone.this.context);
